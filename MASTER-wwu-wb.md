...
- **Status:** **MVP feature-complete** (F0–F8 + audit hardening), merged to `main`, **released** [`v1.0.0-alpha.36`](https://github.com/An-Idea-For-Business/wwu-withdrawal-button/releases/tag/v1.0.0-alpha.36) — in live testing. Current build `1.0.0-alpha.39` (**critical hotfix**: the Settings page fataled with "Class …\Admin\Settings not found" — a missing `use` import; fixed + a 91-file scan confirmed it was the only such case. Also carries the WooCommerce/conflict audit + the `wp_mail_content_type` leak fix. On top of alpha.38 subscription-aware withdrawal, the FluentCart `{{wwu.recesso_url}}` merge-tag, EDD parity, the full security audit). ⚠ FluentCart is shipping a **native EU withdrawal feature** soon — revisit FluentCart-specific investment.

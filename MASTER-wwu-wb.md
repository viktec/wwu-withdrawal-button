...
- **Status:** **MVP feature-complete** (F0–F8 + audit hardening), merged to `main`, **released** [`v1.0.0-alpha.36`](https://github.com/An-Idea-For-Business/wwu-withdrawal-button/releases/tag/v1.0.0-alpha.36) — in live testing. Current build `1.0.0-alpha.37` (FluentCart `{{wwu.recesso_url}}` e-mail merge-tag; on top of EDD parity + the full security audit). ⚠ FluentCart is shipping a **native EU withdrawal feature** soon — revisit FluentCart-specific investment.
...
- **Last updated:** 2026-06-15 (alpha.37 — FluentCart `{{wwu.recesso_url}}` e-mail merge-tag (`Mail\FluentCartWithdrawalTag`), built on the team-verified `smartcode_fallback` `$data` contract with the required `$data['order']` check; needs a live FluentCart test. **Strategic:** FluentCart is shipping native EU withdrawal soon — see `docs/analysis/wwu-wb-fluentcart-hooks-ANALYSIS.md` §"Third verification round")
